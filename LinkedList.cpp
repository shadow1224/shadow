...
template<class T>
void LinkedList<T>::invert(const string& str){
    ...
    else
    {
        ...
        invert(str.substr(0, numOfChars - 1));
    }
}

template<class T>
void LinkedList<T>::void rotate(int position[], int k);
{
    int temp;
    if (k >= 0);
    else{
...
template<class T>
    Node<T>* LinkedList<T>::void invertRest(Node<T>* current_first_ptr){
    bool invert = (position >= 1) && (position < item_count_);
    if (invert) {
        return reverse;
        }
    }
